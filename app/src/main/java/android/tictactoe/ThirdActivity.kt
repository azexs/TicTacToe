...
class ThirdActivity : AppCompatActivity() {
    ...
    private var hehe: MutableList<Int> = mutableListOf()
    ...
    private fun computerMove() {
        val random = hehe.random()

        blocks[random]?.setImageResource(R.drawable.circle)
        blocks[random]?.isEnabled = false
        block[random]="O"
        hehe.remove(random)
